...
function ms(curso, horas, valor) {
    return `${curso} tiene una duración de ${horas} horas y su valor de matrícula es ${valor} pesos`;
}
...
switch (comand) {
    case 'inscribir':
        if (Number(argv.id) < curso.length + 1) {
            ...
            crearArchivo(argv.nombre, argv.cedula, ms(name.curso, name.horas, name.valor))
